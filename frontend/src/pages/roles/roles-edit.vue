...
const readonlyData = ref<Partial<RoleData>>({});
const formErrors = ref<Record<string, string>>({});
const loadingBtn = ref<boolean>(false);

const hasChanges = computed(() => {
  if (!data.value.role || !readonlyData.value.role) return false;

  const currentRoleName =
    typeof data.value.role === "string"
      ? data.value.role
      : data.value.role.name;
  const originalRoleName =
    typeof readonlyData.value.role === "string"
      ? readonlyData.value.role
      : readonlyData.value.role.name;

  if (currentRoleName !== originalRoleName) return true;

  const currentPerms = data.value.rolePermissions || [];
  const originalPerms = readonlyData.value.rolePermissions || [];

  if (currentPerms.length !== originalPerms.length) return true;

  return !originalPerms.every((perm) => currentPerms.includes(perm));
});
...
const loadData = async () => {
  try {
    ...
    data.value = {
      ...
      rolePermissions: [...initialData.rolePermissions],
...
const handleEdit = async () => {
  ...
  try {
    ...
    if (
      ...
      apiMessage.toLowerCase().includes("cargo")
    ) {
    ...
    <div class="mt-12">
      <template v-if="data.role">
        <Input
          ...
          :disabled="
            data.role && (showUser.role === name || name === 'admin')
              ? true
              : false
          "
        ...
        <BaseButton
          :disabled="showUser.role === name || name === 'admin' || !hasChanges"
          @click="handleEdit()"
          :loading="loadingBtn"
