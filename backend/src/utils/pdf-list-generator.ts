...
export async function generateListPDF({
  ...
  const firstColWidth = Math.floor(tableWidth * 0.3);
  const otherColsCount = headers.length - 1;
  const dynamicColWidth =
    otherColsCount > 0
      ? Math.floor((tableWidth - firstColWidth) / otherColsCount)
      : firstColWidth;
    ...
    datas: data.map((item: any) => {
      ...
      headers.forEach((header) => {
        ...
        if (value instanceof Date) {
          ...
          typeof value === "string" &&
          (header.property.includes("at") || header.property.includes("date"))
        ) {
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    doc.text(`${i + 1}`, 30, doc.page.height - 30, {
      align: "right",
